...
public class ClassUtils {
	...
	public static Class<?> newClassInstance(String className) {
		try {
			...
			return null;
//			return BundleClassUtil.newClassInstance(className);
		}
	}
	...
	public static <T> T newClassInstance(Class<T> clazz, String classPath) {
		try {
			Class<?> classTemp = newClassInstance(classPath);
			if (classTemp == null){
				throw new RuntimeException("JavaEval.newClassInstance() Class.forName() found error:", 
						new ClassNotFoundException(classPath));
			}
